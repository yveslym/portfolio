...
if __name__ == "__main__":
    ...
    while win == False and number_of_guessing_left > 0:
        if secret_word != None:
            ...
            if is_guessed_char(user_guess):

                if is_guessed_char_used(user_guess, guessed_word) == False:
                    guessed_word = guessed_word + user_guess

                    result = compare_guessed_char_and_secret_word_char(built_word, user_guess,
                                                                       secret_word)  # true if char match, also return the new built word
                    ...
                    if result[0] == True:  # if guessed character match with one of the character
                        ...
                        number_of_guessing_left = reduce_number_of_guess(
                            number_of_guessing_left)  # reduce number of guessing
                        print (built_word)
